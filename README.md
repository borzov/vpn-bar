...
1. Скачайте последнюю версию приложения (архив `VPNBarApp.zip` и скрипт `install.sh`)
2. Распакуйте архив
3. Запустите скрипт установки из терминала:
...
4. Если появится сообщение о повреждении приложения, выполните в Терминале:

```bash
xattr -dr com.apple.quarantine /Applications/VPNBarApp.app
```
